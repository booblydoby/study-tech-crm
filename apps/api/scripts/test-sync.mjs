      ...
      res.on("end", () => {
        try {
          ...
          if (res.statusCode >= 400) {
            reject(new Error(`${res.statusCode}: ${parsed.message || JSON.stringify(parsed)}`));
          } else {
            resolve(parsed);
          }
        } catch {
...
async function main() {
  try {
    ...
    console.log("   Subjects created: English, Mathematics");
    ...
    console.log("   Has schedule:", !!group1.schedulePattern);
    ...
    console.log("   Has schedule:", !!group2.schedulePattern);
    ...
    groupStudents.forEach((s) => {
      console.log("   -", s.fullName, ":", s.price, "сум");
    });
