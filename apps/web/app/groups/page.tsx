...
export default function GroupsPage() {
  ...
  const [formData, setFormData] = useState({
    name: "", subjectId: "", teacherId: "", monthlyPrice: 0,
    scheduleDays: [1, 3, 5] as number[], scheduleTime: "18:00", scheduleDuration: 90
  });
  ...
  return (
    <AppShell allowedRoles={["ADMIN"]}>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <Button onClick={() => setIsModalOpen(true)}><Plus size={18} />Новая группа</Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filteredGroups.map((g) => (
            <div key={g.id} className="relative block rounded-lg border border-slate-200 bg-white p-4 hover:border-blue-300">
              <Link href={`/groups/${g.id}`} className="block pr-10">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="flex shrink-0 items-center gap-3">
                    ...
                    <span className={`rounded-full px-2 py-1 text-xs ${g.status === "ACTIVE" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-600"}`}>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Новая группа" footer={null}>
        <form onSubmit={handleCreate} className="space-y-4">
          <p className="text-sm text-slate-500">Групповое занятие — несколько учеников по общему расписанию. Для индивидуальных занятий используйте раздел «Студенты».</p>
          <div>
            <label className="mb-1 block text-sm font-medium text-slate-700">Название группы *</label>
            <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="Напр. English A1" required />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="mb-1 block text-sm font-medium text-slate-700">Предмет *</label>
              <select value={formData.subjectId} onChange={(e) => setFormData({ ...formData, subjectId: e.target.value })} className="w-full rounded-md border px-3 py-2" required>
            ...
            <div>
              ...
              <select value={formData.teacherId} onChange={(e) => setFormData({ ...formData, teacherId: e.target.value })} className="w-full rounded-md border px-3 py-2" required>
          ...
          <div>
            ...
            <Input type="number" value={formData.monthlyPrice} onChange={(e) => setFormData({ ...formData, monthlyPrice: parseInt(e.target.value, 10) || 0 })} placeholder="Напр. 500000" required />
          </div>
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input type="number" value={formData.scheduleDuration} onChange={(e) => setFormData({ ...formData, scheduleDuration: parseInt(e.target.value, 10) || 90 })} placeholder="90" />
              </div>
            </div>
          </div>
